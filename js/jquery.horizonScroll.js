...
(function ($) {
    ...
    $.fn.horizon = function (options, i) {
        if (options === 'scrollLeft') {
            ...
            $('body,html').on('mousewheel MozMousePixelScroll DOMMouseScroll', function (e) {
                e.preventDefault();
                ...
                var delta = (evt.detail || evt.detail != 0) ? evt.detail * (-40) : evt.wheelDelta;
                // if delta stays undefined or zero - try to find it in evt.deltaY
                var deltaCalculated = delta && delta != 0;
                // all ok, stay like it is
                if (!deltaCalculated && evt.deltaY) {
                    delta = evt.deltaY * (-40);
                }
                ...
                scrollRight();
            }).on('click', '_.navigation a', function () {
                var iddata =   $(this).attr("data");
                scrollRightsub(iddata);
...
// http://stackoverflow.com/questions/8790752/callback-of-animate-gets-called-twice-jquery
    ...
    var scrollRightsub = function (iddata) {
